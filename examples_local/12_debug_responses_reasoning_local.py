...
# Use proxy key + openai/ prefix routed via litellm proxy
api_key = os.getenv("LITELLM_API_KEY")
assert api_key, "LITELLM_API_KEY environment variable is not set."
MODEL = os.getenv("DEBUG_MODEL", "openai/gpt-5-mini-2025-08-07")
...
conv.send_message(
    message=Message(
        ...
        content=[
            TextContent(
                text=(
                    "Echo 'Hello' using a tool call. Also explain briefly why you're "
                    "doing it."
                )
            )
        ],
...
artifact_path = os.path.join(LOG_DIR, "12_debug_responses_reasoning_local.last.json")
meta = {
    "model": MODEL,
...
print("Saved debug artifact to:", artifact_path)
print("Model:", MODEL)
print("Responses supported:", meta["supports_responses"])
