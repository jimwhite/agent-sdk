...
class BaseAgentServer(ABC):
    """Abstract base class for running OpenHands Agent Server in different environments.

    This class defines the interface that all agent server implementations
    must follow. Implementations can be local subprocess-based, Docker-based,
    remote API-based, or other
    ...
    @abstractmethod
    def __enter__(self) -> BaseAgentServer:
