...
class MockEventsToMessagesAction(ActionBase):
    """Mock action for testing."""

    command: str


class MockEventsToMessagesObservation(ObservationBase):
...
def create_action_event(
    ...
    action = MockEventsToMessagesAction(command=action_args.get("command", "test"))
...
class TestEventsToMessages:
    ...
    def test_parallel_function_calling_same_response_id(self):
        ...
        action2 = ActionEvent(
            ...
            action=MockEventsToMessagesAction(command="test"),
            tool_name="get_current_weather",
    ...
    def test_mixed_event_types(self):
        ...
        observation_event = ObservationEvent(
            source="environment",
            observation=MockEventsToMessagesObservation(result="Sunny, 72°F"),
    ...
    def test_complex_parallel_and_sequential_mix(self):
        ...
        obs_sf = ObservationEvent(
            source="environment",
            observation=MockEventsToMessagesObservation(result="SF: Sunny, 65°F"),
            action_id="action_sf",
            tool_name="get_weather",
            tool_call_id="call_sf_weather",
        )
        ...
        obs_nyc = ObservationEvent(
            source="environment",
            observation=MockEventsToMessagesObservation(result="NYC: Cloudy, 45°F"),
