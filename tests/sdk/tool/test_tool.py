...
class MockToolAction(ActionBase):
...
class MockToolObservation(ObservationBase):
...
class TestTool:
    ...
    def test_tool_creation_basic(self):
        ...
        tool = Tool(
            ...
            action_type=MockToolAction,
            observation_type=MockToolObservation,
        )

        assert tool.name == "test_tool"
        assert tool.description == "A test tool"
        assert tool.action_type == MockToolAction
        assert tool.observation_type == MockToolObservation
    ...
    def test_tool_creation_with_executor(self):
        ...
        class MockExecutor(ToolExecutor):
            def __call__(self, action: MockToolAction) -> MockToolObservation:
                return MockToolObservation(result=f"Executed: {action.command}")
        ...
        action = MockToolAction(command="test")
        result = executable_tool(action)
        assert isinstance(result, MockToolObservation)
    ...
    def test_call_with_executor(self):
        ...
        class MockExecutor(ToolExecutor):
            def __call__(self, action: MockToolAction) -> MockToolObservation:
                return MockToolObservation(result=f"Processed: {action.command}")
        ...
        action = MockToolAction(command="test_command")
        result = tool(action)
    ...
    def test_observation_type_validation(self):
        ...
        class MockExecutor(ToolExecutor):
            def __call__(self, action: MockToolAction) -> MockToolObservation:
                return MockToolObservation(result="success")
    ...
    def test_observation_with_extra_fields(self):
        ...
        class MockExecutor(ToolExecutor):
            def __call__(self, action: MockToolAction) -> MockToolObservation:
                return MockToolObservation(result="test", extra_field="extra_data")
    ...
    def test_action_validation_with_nested_data(self):
        ...
        assert isinstance(action, MockToolAction)
    ...
    def test_schema_roundtrip_conversion(self):
        ...
        # Start with a class
        original_schema = MockToolAction.to_mcp_schema()
    ...
    def test_executor_function_attachment(self):
        ...
        class MockExecutor(ToolExecutor):
            def __call__(self, action: MockToolAction) -> MockToolObservation:
                return MockToolObservation(result=f"Attached: {action.command}")
    ...
    def test_complex_executor_return_types(self):
        ...
        class MockComplexExecutor(ToolExecutor):
            def __call__(self, action: MockToolAction) -> ComplexObservation:
    ...
    def test_executor_with_observation_validation(self):
        ...
        class ValidExecutor(ToolExecutor):
            def __call__(self, action: MockToolAction) -> StrictObservation:
    ...
    def test_security_risk_is_required_field_in_schema(self):
        ...
        action_type_with_risk = _create_action_type_with_risk(MockToolAction)
