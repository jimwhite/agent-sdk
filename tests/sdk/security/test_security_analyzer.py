...
class MockSecurityAnalyzerAction(ActionBase):
    """Mock action for testing."""

    command: str = "test_command"


class DummySecurityAnalyzer(SecurityAnalyzerBase):
...
def test_analyze_event_with_action_event():
    ...
    analyzer = DummySecurityAnalyzer(risk_return_value=SecurityRisk.MEDIUM)
    action = MockSecurityAnalyzerAction(command="test")
...
def test_analyze_event_with_non_action_event():
    ...
    analyzer = DummySecurityAnalyzer(risk_return_value=SecurityRisk.HIGH)
...
def test_analyze_pending_actions_success():
    ...
    action1 = MockSecurityAnalyzerAction(command="action1")
    action2 = MockSecurityAnalyzerAction(command="action2")
...
def test_analyze_pending_actions_empty_list():
    ...
    analyzer = DummySecurityAnalyzer(risk_return_value=SecurityRisk.LOW)
...
def test_analyze_pending_actions_with_exception():
    ...
    class FailingAnalyzer(DummySecurityAnalyzer):
        def security_risk(self, action: ActionEvent) -> SecurityRisk:
            super().security_risk(action)  # Record the call
            raise ValueError("Analysis failed")

    analyzer = FailingAnalyzer()
    action = MockSecurityAnalyzerAction(command="failing_action")
    action_event = create_mock_action_event(action)
...
def test_analyze_pending_actions_mixed_risks() -> None:
    ...
    class VariableRiskAnalyzer(DummySecurityAnalyzer):
        call_count: int = 0
    ...
    actions = [MockSecurityAnalyzerAction(command=f"action{i}") for i in range(3)]
...
def test_analyze_pending_actions_partial_failure():
    ...
    class PartiallyFailingAnalyzer(DummySecurityAnalyzer):
        def security_risk(self, action: ActionEvent) -> SecurityRisk:
    ...
    action1 = MockSecurityAnalyzerAction(command="good_action")
    action2 = MockSecurityAnalyzerAction(command="failing_action")
    action3 = MockSecurityAnalyzerAction(command="another_good_action")
