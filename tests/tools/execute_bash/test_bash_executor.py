...
from openhands.tools.execute_bash.definition import ExecuteBashDataConverter
from openhands.tools.execute_bash.impl import BashExecutor
from openhands.tools.execute_bash.terminal import TerminalCommandStatus
...
def test_session_initialization(terminal_type):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        obs = executor(ExecuteBashAction(command="pwd"))

        assert temp_dir in obs.data.get("output", "")
        assert "[The command completed with exit code 0.]" in obs.data.get(
            "metadata", {}
        ).get("suffix", "")
        executor.close()
    ...
    executor = BashExecutor(
        working_dir=os.getcwd(), username="nobody", terminal_type=terminal_type
    )
    executor.close()
...
def test_cwd_property(tmp_path, terminal_type):
    executor = BashExecutor(working_dir=tmp_path, terminal_type=terminal_type)

    # Change directory and verify pwd updates
    random_dir = tmp_path / "random"
    random_dir.mkdir()
    executor(ExecuteBashAction(command=f"cd {random_dir}"))

    # For other implementations, just verify the command executed successfully
    obs = executor(ExecuteBashAction(command="pwd"))
    assert str(random_dir) in obs.data.get("output", "")
    ...
    assert executor.session.cwd == str(random_dir)


@parametrize_terminal_types
def test_basic_command(terminal_type):
    executor = BashExecutor(working_dir=os.getcwd(), terminal_type=terminal_type)

    # Test simple command
    obs = executor(ExecuteBashAction(command="echo 'hello world'"))

    assert "hello world" in obs.data.get("output", "")
    assert (
        obs.data.get("metadata", {}).get("suffix", "")
        == "\n[The command completed with exit code 0.]"
    )
    # Note: prefix may vary between terminal implementations
    assert obs.data.get("metadata", {}).get("exit_code", 0) == 0
    assert executor.session.prev_status == TerminalCommandStatus.COMPLETED

    # Test command with error
    obs = executor(ExecuteBashAction(command="nonexistent_command"))
    ...
    assert "nonexistent_command: command not found" in obs.data.get("output", "")
    assert executor.session.prev_status == TerminalCommandStatus.COMPLETED
    ...
    assert "first" in obs.data.get("output", "")
    assert "second" in obs.data.get("output", "")
    assert "third" in obs.data.get("output", "")
    assert (
        obs.data.get("metadata", {}).get("suffix", "")
        == "\n[The command completed with exit code 0.]"
    )
...
def test_long_running_command_follow_by_execute():
    executor = BashExecutor(working_dir=os.getcwd(), no_change_timeout_seconds=2)
    session = executor.session
...
def test_empty_command_error(terminal_type):
    ...
    obs = executor(ExecuteBashAction(command=""))

    assert obs.data.get("error", False) is True
    assert (
        obs.data.get("output", "")
    ...
    assert "There was an error during command execution." in agent_obs[0].text
    assert (
        "ERROR: No previous running command to retrieve logs from." in agent_obs[0].text
    )
    assert obs.data.get("metadata", {}).get("exit_code") == -1
    assert obs.data.get("metadata", {}).get("prefix") == ""
    assert obs.data.get("metadata", {}).get("suffix") == ""
    assert executor.session.prev_status is None
...
def test_multiline_command():
    executor = BashExecutor(working_dir=os.getcwd())
    session = executor.session
...
def test_bash_server(terminal_type):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        executor = BashExecutor(working_dir=temp_dir, terminal_type=terminal_type)
        session = executor.session
        try:
