...
INSTRUCTION = (
    "Use Jupyter IPython to write a text file containing 'hello world' to "
    "'/workspace/test.txt'."
)
...
class JupyterWriteFileTest(BaseIntegrationTest):
    ...
    def verify_result(self) -> TestResult:
        ...
        if not os.path.exists(file_path):
            return TestResult(
                success=False, reason="Text file not found at /workspace/test.txt"
            )
        ...
        return TestResult(
            success=True,
            reason="Successfully created text file with 'hello world' using IPython",
        )
